...
def get_data(client_settings, method):

    '''
    The function is used to get the previous day's organizations or people data from Crunchbase.
    The inputs should be a dictionary (client_settings) and a string (method).
    ...
    cf.read("rapidapikey.ini")
    RAPIDAPI_KEY = cf.get('rapid_api_key','key')


    # Define a function for triggering Crunchbase API
    def trigger_api(settings,since_time):   #settings is a dic that contains clients settings
        ...
        headers = { 'x-rapidapi-host': "crunchbase-crunchbase-v1.p.rapidapi.com",
                    'x-rapidapi-key': "d14dc13be7mshe38454abb0d95dbp16abdajsn159db976e43b"}
    ...
    def get_rest_pages(settings, since_time, i):  
    ...
    def get_rest_pages_gen(settings,since_time, pages):  
    ...
    def result_by_threads(settings):
        ...
        try: 
            ...
            [data,pages] = trigger_api(settings,yday_timestamp_utc)  
        
            # Use multithreading to integrate the dataframe
            with concurrent.futures.ThreadPoolExecutor() as executor:
                threads = [executor.submit(get_rest_pages,settings,yday_timestamp_utc,i) for i in range(pages//10+1)]
    ...
    def result_by_generator(settings):
        ...
        try:
            ...
            [first_page,pages] = trigger_api(settings, yday_timestamp_utc)    
        
            # Use generator to integrate the dataframe
            rest_pages = get_rest_pages_gen(settings, yday_timestamp_utc, pages)
    ...
    if method == "threads":
        data = result_by_threads(client_settings)
        print("The result is in one dataframe.")
        return data    # a dataframe
    else:
        [first_page,rest_pages] = result_by_generator(client_settings)
